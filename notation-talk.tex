...
\item meaning we could not evaluate the results or conclusions
...
\blue{Continuous} variable $X$:
\bi
\item $f(x)$ -- \blue{continuous} probability density function
...
\nonumber f(x) = \frac{1}{\sqrt{2 \pi} \sigma x} \mbox{e}^{-(\log x - \mu)^2 / 2
  \sigma^2}, ~~ x > 0
\ee
for parameters $\mu$ and $\sigma$, and $\log$ is \red{natural logarithm} -- worth specifying and using $\log_{10}$ for
